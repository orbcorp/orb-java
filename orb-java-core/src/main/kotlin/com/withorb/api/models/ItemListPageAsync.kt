...
class ItemListPageAsync private constructor(private val itemsService: ItemServiceAsync, private val params: ItemListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<ItemListParams> {
      ...
      return Optional.of(ItemListParams.builder().from(params).apply {paginationMetadata().nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<ItemListPageAsync>> {
      return getNextPageParams().map {
        itemsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: ItemListPageAsync, ) {

        fun forEach(action: Predicate<Item>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<ItemListPageAsync>>.forEach(action: (Item) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
