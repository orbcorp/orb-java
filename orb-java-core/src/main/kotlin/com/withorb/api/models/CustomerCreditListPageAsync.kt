...
class CustomerCreditListPageAsync private constructor(private val creditsService: CreditServiceAsync, private val params: CustomerCreditListParams, private val response: Response, ) {
    ...
    fun getNextPage(): CompletableFuture<Optional<CustomerCreditListPageAsync>> {
      return getNextPageParams().map {
        creditsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: CustomerCreditListPageAsync, ) {

        fun forEach(action: Predicate<CustomerCreditListResponse>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CustomerCreditListPageAsync>>.forEach(action: (CustomerCreditListResponse) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
