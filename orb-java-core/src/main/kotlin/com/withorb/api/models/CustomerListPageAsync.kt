...
class CustomerListPageAsync private constructor(private val customersService: CustomerServiceAsync, private val params: CustomerListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<CustomerListParams> {
      ...
      return Optional.of(CustomerListParams.builder().from(params).apply {paginationMetadata().nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<CustomerListPageAsync>> {
      return getNextPageParams().map {
        customersService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: CustomerListPageAsync, ) {

        fun forEach(action: Predicate<Customer>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CustomerListPageAsync>>.forEach(action: (Customer) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
