...
class CouponSubscriptionListPageAsync private constructor(private val subscriptionsService: SubscriptionServiceAsync, private val params: CouponSubscriptionListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<CouponSubscriptionListParams> {
      ...
      return Optional.of(CouponSubscriptionListParams.builder().from(params).apply {paginationMetadata().nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<CouponSubscriptionListPageAsync>> {
      return getNextPageParams().map {
        subscriptionsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: CouponSubscriptionListPageAsync, ) {

        fun forEach(action: Predicate<Subscription>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CouponSubscriptionListPageAsync>>.forEach(action: (Subscription) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
