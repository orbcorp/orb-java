...
class PriceListPageAsync private constructor(private val pricesService: PriceServiceAsync, private val params: PriceListParams, private val response: Response, ) {
    ...
    class AutoPager constructor(private val firstPage: PriceListPageAsync, ) {

        fun forEach(action: Predicate<Price>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<PriceListPageAsync>>.forEach(action: (Price) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
