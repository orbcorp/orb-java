...
class CustomerCreditLedgerListByExternalIdPageAsync private constructor(private val ledgerService: LedgerServiceAsync, private val params: CustomerCreditLedgerListByExternalIdParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<CustomerCreditLedgerListByExternalIdParams> {
      ...
      return Optional.of(CustomerCreditLedgerListByExternalIdParams.builder().from(params).apply {paginationMetadata().nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<CustomerCreditLedgerListByExternalIdPageAsync>> {
      return getNextPageParams().map {
        ledgerService.listByExternalId(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class Response constructor(private val data: JsonField<List<CustomerCreditLedgerListByExternalIdResponse>>, private val paginationMetadata: JsonField<PaginationMetadata>, private val additionalProperties: Map<String, JsonValue>, ) {
        ...
        @JsonProperty("data")
        fun _data(): Optional<JsonField<List<CustomerCreditLedgerListByExternalIdResponse>>> = Optional.ofNullable(data)
    ...
    class AutoPager constructor(private val firstPage: CustomerCreditLedgerListByExternalIdPageAsync, ) {

        fun forEach(action: Predicate<CustomerCreditLedgerListByExternalIdResponse>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CustomerCreditLedgerListByExternalIdPageAsync>>.forEach(action: (CustomerCreditLedgerListByExternalIdResponse) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
        ...
        fun toList(executor: Executor): CompletableFuture<List<CustomerCreditLedgerListByExternalIdResponse>> {
