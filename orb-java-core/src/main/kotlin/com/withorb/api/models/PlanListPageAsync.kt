...
class PlanListPageAsync private constructor(private val plansService: PlanServiceAsync, private val params: PlanListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<PlanListParams> {
      ...
      return Optional.of(PlanListParams.builder().from(params).apply {paginationMetadata().nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<PlanListPageAsync>> {
      return getNextPageParams().map {
        plansService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: PlanListPageAsync, ) {

        fun forEach(action: Predicate<Plan>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<PlanListPageAsync>>.forEach(action: (Plan) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
