...
class AlertListPageAsync private constructor(private val alertsService: AlertServiceAsync, private val params: AlertListParams, private val response: Response, ) {
    ...
    fun getNextPage(): CompletableFuture<Optional<AlertListPageAsync>> {
      return getNextPageParams().map {
        alertsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: AlertListPageAsync, ) {

        fun forEach(action: Predicate<Alert>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<AlertListPageAsync>>.forEach(action: (Alert) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
