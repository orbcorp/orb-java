...
class DimensionalPriceGroupListPageAsync private constructor(
    ...
    fun getNextPageParams(): Optional<DimensionalPriceGroupListParams> {
      ...
      return Optional.of(DimensionalPriceGroupListParams.builder().from(params).apply {paginationMetadata().nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<DimensionalPriceGroupListPageAsync>> {
      return getNextPageParams().map {
        dimensionalPriceGroupsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<DimensionalPriceGroup>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<DimensionalPriceGroupListPageAsync>>.forEach(action: (DimensionalPriceGroup) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
