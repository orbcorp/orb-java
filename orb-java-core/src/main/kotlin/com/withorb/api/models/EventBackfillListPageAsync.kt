...
class EventBackfillListPageAsync private constructor(private val backfillsService: BackfillServiceAsync, private val params: EventBackfillListParams, private val response: Response, ) {
    ...
    fun getNextPageParams(): Optional<EventBackfillListParams> {
      ...
      return Optional.of(EventBackfillListParams.builder().from(params).apply {paginationMetadata().nextCursor().ifPresent{ this.cursor(it) } }.build())
    }

    fun getNextPage(): CompletableFuture<Optional<EventBackfillListPageAsync>> {
      return getNextPageParams().map {
        backfillsService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class Response constructor(private val data: JsonField<List<EventBackfillListResponse>>, private val paginationMetadata: JsonField<PaginationMetadata>, private val additionalProperties: Map<String, JsonValue>, ) {
        ...
        @JsonProperty("data")
        fun _data(): Optional<JsonField<List<EventBackfillListResponse>>> = Optional.ofNullable(data)
    ...
    class AutoPager constructor(private val firstPage: EventBackfillListPageAsync, ) {

        fun forEach(action: Predicate<EventBackfillListResponse>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<EventBackfillListPageAsync>>.forEach(action: (EventBackfillListResponse) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
