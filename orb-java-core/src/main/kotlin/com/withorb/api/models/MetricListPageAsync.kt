...
class MetricListPageAsync private constructor(private val metricsService: MetricServiceAsync, private val params: MetricListParams, private val response: Response, ) {
    ...
    class AutoPager constructor(private val firstPage: MetricListPageAsync, ) {

        fun forEach(action: Predicate<BillableMetric>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<MetricListPageAsync>>.forEach(action: (BillableMetric) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
