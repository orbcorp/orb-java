...
class CustomerCreditLedgerListPageAsync private constructor(
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<CustomerCreditLedgerListResponse>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CustomerCreditLedgerListPageAsync>>.forEach(action: (CustomerCreditLedgerListResponse) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
