...
class CreditNoteListPageAsync private constructor(private val creditNotesService: CreditNoteServiceAsync, private val params: CreditNoteListParams, private val response: Response, ) {
    ...
    fun getNextPage(): CompletableFuture<Optional<CreditNoteListPageAsync>> {
      return getNextPageParams().map {
        creditNotesService.list(it).thenApply { Optional.of(it) }
      }.orElseGet {
    ...
    class AutoPager constructor(private val firstPage: CreditNoteListPageAsync, ) {

        fun forEach(action: Predicate<CreditNote>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CreditNoteListPageAsync>>.forEach(action: (CreditNote) -> Boolean, executor: Executor): CompletableFuture<Void> = thenComposeAsync({ page -> 
              page
              .filter {
                  it.data().all(action)
              }
