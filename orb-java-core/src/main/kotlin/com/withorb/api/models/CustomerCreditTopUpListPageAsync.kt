...
class CustomerCreditTopUpListPageAsync private constructor(
    ...
    class AutoPager(
        ...
        fun forEach(action: Predicate<CustomerCreditTopUpListResponse>, executor: Executor): CompletableFuture<Void> {
          fun CompletableFuture<Optional<CustomerCreditTopUpListPageAsync>>.forEach(action: (CustomerCreditTopUpListResponse) -> Boolean, executor: Executor): CompletableFuture<Void> =
              thenComposeAsync(
                { page ->
                    page
                    .filter {
